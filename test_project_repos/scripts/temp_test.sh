...
EOF
    ...
    if command -v classroom-pilot &> /dev/null; then
        ...
        if classroom-pilot assignments setup --config-file "$config_file" --dry-run &> /dev/null; then
            mark_test_passed "Assignment setup dry-run"
        else
            mark_test_failed "Assignment setup dry-run" "Dry-run failed"
        fi
    else
...
EOF
    ...
    if python3 "$isolated_test_script"; then
...
test_logging_output() {
    ...
    # Test verbose output
    if classroom-pilot --version --verbose > "$log_file" 2>&1; then
        if grep -q "verbose" "$log_file" || [[ -s "$log_file" ]]; then
            mark_test_passed "Verbose logging output"
        else
            mark_test_failed "Verbose logging output" "No verbose output detected"
        fi
    else
